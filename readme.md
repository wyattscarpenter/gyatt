...
Gyatt sets core.whitespace to incomplete-line. The effect of this is that (all other whitespace error warning settings remain unchanged, and) diff --check will now actually warn you when a text file ends without a trailing newline. This fixes an old pet peeve of mine: git would complain at you when committing a file that had no newline at the end of the file; but it would not warn about this in git diff --check. This feature might only be enabled on more recent gits; https://git-scm.com/docs/git-config/2.53.0#Documentation/git-config.txt-corewhitespace (2016-02-02) has it but https://git-scm.com/docs/git-config/2.52.0#Documentation/git-config.txt-corewhitespace (2025-11-17) doesn't. My git is 2.52.0 and it just silently ignores this. I hope it works in the future versions as described, though. (Come to think of it, future versions of Git may change/have-changed the other half of the pet peeve behavior, as well. Who knows.) Anyway, there's a good case to be made that, actually, you should only care/track whitespace "errors" in files to the exclusive end of minimizing diffs; maximum strictness in not emitting them is one way to achieve this — but so is maximum laxness in all places that would display them, which would theoretically keep you from wasting time on these trivialities.
...
* It would maybe be sort of useful to disable git stash since one can easily fall into using it too persistently and footgun oneself into further inconvenience. (You're supposed to use a feature branch for this sort of thing to get the most cromulent UX.) On the other hand, it's pretty annoying to not be able to git stash things momentarily — its intended use.
